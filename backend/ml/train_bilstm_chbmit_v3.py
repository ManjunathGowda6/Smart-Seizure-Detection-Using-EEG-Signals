"""
train_bilstm_chbmit_v3.py
=========================
Retrain CNN-BiLSTM on CHB-MIT cached numpy arrays (Version 3).

Optimizations in V3:
  - pos_weight = 2.0 (less aggressive seizure bias)
  - SMOTE ratio = 0.3 (seizure windows become 30% of normal windows)
  - learning rate = 5e-4, patience = 20
  - THRESHOLD = 0.50

SAVED MODEL
  backend/models/cnn_bilstm_chbmit_v3.pt
"""
...
SAVE_PATH  = os.path.join(MODELS_DIR, 'cnn_bilstm_chbmit_v3.pt')
...
LR                   = 5e-4
WEIGHT_DECAY         = 1e-4
POS_WEIGHT           = 2.0     # BCEWithLogitsLoss weight for seizure class
SMOTE_RATIO          = 0.3     # target seizure:normal = 0.3 after SMOTE
SMOTE_K              = 5       # k-nearest neighbours for SMOTE
THRESHOLD            = 0.50    # sigmoid threshold for classification
EARLY_STOP_PATIENCE  = 20
DEVICE               = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# ══════════════════════════════════════════════════════════════════════════════
print('=' * 65)
print('  CNN-BiLSTM  |  CHB-MIT Domain Retraining (Version 3)')
print('=' * 65)
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 3 — SMOTE to balance training sub-windows (seizure:normal = 0.3 after SMOTE)
# ══════════════════════════════════════════════════════════════════════════════
...
print(f'  False Positive R.: {fpr:.4f}  (FP={fp:,} out of {fp+tn:,} normal windows)')
print()
if recall >= 0.75 and precision >= 0.40:
    print(f'  [SUCCESS] Recall {recall:.2%} and Precision {precision:.2%} meet targets')
else:
    print(f'  [WARNING] Metrics do not meet targets (Target: Recall > 75%, Precision > 40%)')
print('=' * 60)
...
for f in ['cnn_bilstm_v1.pt', 'cnn_bilstm_chbmit.pt', 'cnn_bilstm_chbmit_v2.pt', 'cnn_bilstm_chbmit_v3.pt', 'cnn_lti_model.pt']:
