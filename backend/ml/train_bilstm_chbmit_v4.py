"""
train_bilstm_chbmit_v4.py
=========================
Retrain CNN-BiLSTM on CHB-MIT cached numpy arrays (Version 4).

Optimizations in V4:
  - Remove SMOTE completely (avoid linear interpolation artifacts in EEG)
  - pos_weight = 4.0
  - learning rate = 5e-4, patience = 20
  - THRESHOLD = 0.50

SAVED MODEL
  backend/models/cnn_bilstm_chbmit_v4.pt
"""
...
SAVE_PATH  = os.path.join(MODELS_DIR, 'cnn_bilstm_chbmit_v4.pt')
...
LR                   = 5e-4
WEIGHT_DECAY         = 1e-4
POS_WEIGHT           = 4.0     # BCEWithLogitsLoss weight for seizure class
...
# ══════════════════════════════════════════════════════════════════════════════
print('=' * 65)
print('  CNN-BiLSTM  |  CHB-MIT Domain Retraining (Version 4)')
print('=' * 65)
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 3 — Disable SMOTE (use raw unbalanced proportions)
# ══════════════════════════════════════════════════════════════════════════════
print('\n[STEP 3] Skipping SMOTE (using raw patient class distribution) ...')
X_bal = X_sub_train
y_bal = y_sub_train
print(f'  Total : {len(X_bal):,} training sub-windows (seizure={y_bal.sum():,}, normal={(y_bal==0).sum():,})')
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 8 — Evaluate on CHB-MIT test set (real distribution)
# ══════════════════════════════════════════════════════════════════════════════
...
for f in ['cnn_bilstm_v1.pt', 'cnn_bilstm_chbmit.pt', 'cnn_bilstm_chbmit_v2.pt', 'cnn_bilstm_chbmit_v3.pt', 'cnn_bilstm_chbmit_v4.pt', 'cnn_lti_model.pt']:
